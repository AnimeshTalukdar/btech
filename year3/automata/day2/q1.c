...
void fun(int idx,int n,int state,int* state_changes,int* arr,int values){
    if(idx == n && (state == 1 || state == 2 || state == 3)){
...
int main(){
    ...
    state_changes[1+values*0]=2;
    state_changes[0+values*1]=0;
    state_changes[1+values*1]=3;
    
    
    state_changes[0+values*2]=3;
    state_changes[1+values*2]=0;

    state_changes[0+values*3]=2;
    state_changes[1+values*3]=1;
