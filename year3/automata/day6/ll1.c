...
char * substring(char * string,int begin,int num){
	...
	int i; for (i = begin; i < begin+num; ++i)
	{
		res[j] = string[i];
		j++;
	}
...
char *removeDuplicated(char table[]){
	...
	int i; for (i = 0; i < strlen(table); ++i)
	{
		int exist =0;
        int j;
		for (j = i+1; j < strlen(table); ++j)
		{
				if(table[i] == table[j]){
					exist = 1;
				}
...
rules * removeRecursion (rules * p){
	...
	while(iterator != NULL){
		if(checkForRecursion(iterator)){
			...
			int k = 0;
			int l = 0;
			int i; for (i = 0; i < iterator->count; ++i)
			{
...
int checkForEpsilon(char first[]){
	int i; for (i = 0; i < strlen(first); ++i)
	{
		if(first[i] == '&'){
			return 1;
...
int checkIfNameExist(char production[],char name[]){
	if(strlen(name) == 2 ){
		int i; for (i = 0; i < strlen(production)-1; ++i)
		{
			if(name[0] == production[i] && name[1] == production[i+1])
				return (i+2);
		}
	}else{
		int i; for (i = 0; i < strlen(production); ++i)
		{
...
void printfirst(rules * p){
	while(p !=NULL){
		...
		int i; for (i = 0; i < p->first_count; ++i)
		{
...
void printfollow(rules * p){
	while(p !=NULL){
		...
		int i; for (i = 0; i < p->follow_count; ++i)
		{
